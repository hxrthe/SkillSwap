...
// Initialize crud and get pending requests
$crud = new Crud();
$pendingRequests = $crud->getPendingCommunityRequests();
...
<body>
    ...
    <div class="sidebar">
        <ul>
            ...
            <li><i class="fas fa-users"></i><a href="#" onclick="showRequests()">Requests</a></li>
    ...
    <!-- Requests Layout -->
    <div class="requests-container" style="display: none; margin-left: 240px; margin-top: 70px; padding: 20px;">
        <h2 style="margin-bottom: 20px;">Community Creation Requests</h2>
        <div class="requests-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
            <?php if (!empty($pendingRequests)): ?>
                <?php foreach ($pendingRequests as $request): ?>
                    <div class="request-card" data-request-id="<?php echo htmlspecialchars($request['Community_ID']); ?>" style="background-color: #fff; border-radius: 10px; padding: 20px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                        <div class="request-details" style="margin-bottom: 15px;">
                            <h3 style="margin: 0 0 10px;">Community Name: <?php echo htmlspecialchars($request['name']); ?></h3>
                            <p style="margin: 0 0 10px; color: #666;"><?php echo htmlspecialchars($request['topic']); ?></p>
                            <div class="interests" style="display: flex; gap: 10px; flex-wrap: wrap;">
                                <span style="background-color: #f0f0f0; padding: 5px 10px; border-radius: 15px; font-size: 0.9em;"><?php echo htmlspecialchars($request['interest1']); ?></span>
                                <span style="background-color: #f0f0f0; padding: 5px 10px; border-radius: 15px; font-size: 0.9em;"><?php echo htmlspecialchars($request['interest2']); ?></span>
                                <span style="background-color: #f0f0f0; padding: 5px 10px; border-radius: 15px; font-size: 0.9em;"><?php echo htmlspecialchars($request['interest3']); ?></span>
                            </div>
                        </div>
                        <div class="request-actions" style="display: flex; gap: 10px;">
                            <button onclick="handleRequest(<?php echo $request['Community_ID']; ?>, 'approve')" style="background-color: #4CAF50; color: white; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer;">Approve</button>
                            <button onclick="handleRequest(<?php echo $request['Community_ID']; ?>, 'decline')" style="background-color: #f44336; color: white; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer;">Decline</button>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
    ...
    <script>
        ...
        function showRequests() {
            ...
            document.querySelector('.messages-container').style.display = 'none';
            document.querySelector('.dashboard-container').style.display = 'none';
            document.querySelector('.requests-container').style.display = 'block';
        }

        function handleRequest(requestId, action) {
            const message = action === 'approve' ? 
                'Are you sure you want to approve this community request?' : 
                'Are you sure you want to decline this community request?';
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('handle_community_request.php', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            Swal.fire({
                                title: 'Success!',
                                text: action === 'approve' ? 'Community request approved!' : 'Community request declined!',
                                icon: 'success',
